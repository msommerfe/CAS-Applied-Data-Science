...
#Left join on 'Date'. the dfPubliBikeAvailability will be enriched with all date from df_weatherDataBern
dfPubliBikeAvailability = dfPubliBikeAvailability.merge(dfWeatherDataBern, on=['date'], how = 'left')

#plots some plots to get an overview of the data
plotDataOverviewPubliBikeAvailability(dfPubliBikeAvailability)
...
#Train the model
availability = model.fit(dfFeatureTrainPB, dfLabelsTrainPB, epochs=1000, batch_size=200, validation_data=(dfFeaturesTestPB, dfLabelsTestPB))
